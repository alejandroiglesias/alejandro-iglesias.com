;(function() {
  ...
  const vala = $('body').vala({ cls: 'mark' }, function(event, highlightId, range, done) {
  ...
  if (urlParams.get('hl')) {
    ...
    if (highlightConfig.uuid !== uuid) {
      ...
      for (const elementId in highlightConfig.highlights) {
        ...
        elementDom.html(vala.render(elementDom.text(), Array.from(elementEntry.values())))
      }
  ...
  const updateUrlParams = () => {
    urlParams.set('hl', btoa(JSON.stringify(Object.assign(highlightConfig, { uuid }))))
    window.history.replaceState({}, '', `${location.pathname}?${urlParams}`)
  }
